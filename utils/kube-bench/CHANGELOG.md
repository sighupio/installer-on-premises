...
The table below reflects the full set of customizations currently in effect across all supported CIS versions.
...
| 1.2.11              | Skipped  | `AlwaysPullImages` cannot be enforced by default by the installer; from K8s 1.35 the security concern this check addresses is natively mitigated by `KubeletEnsureSecretPulledImages` (KEP-2862, on by default since 1.35)  |
...
| 1.2.1 | Anonymous authentication set to false                   | INFO   | Anonymous authentication used for metrics scraping is safe under proper RBAC |
| 1.2.11| Admission plugin AlwaysPullImages                       | INFO   | AlwaysPullImages cannot be enforced by default by the installer |
...
## Changelog per CIS version

### cis-1.12 (Kubernetes 1.32, 1.33, 1.34, 1.35)

Changes introduced with respect to cis-1.11:

- 1.1.7 / 1.1.8: added skip (etcd runs as systemd, no pod spec file; not applicable in our setup)
- 1.1.9 / 1.1.10: CNI path customization carried forward; upstream changed path to `/var/lib/cni/networks`, we keep `/var/lib/cni`
- 1.2.11: added note that from K8s 1.35 the security concern is natively mitigated by `KubeletEnsureSecretPulledImages` (KEP-2862)
- 4.1.3 / 4.1.4: added skip (kube-proxy runs as DaemonSet, no kubeconfig file on nodes)
- 4.2.15: not present and removed by upstream in cis-1.12

### cis-1.11 (Kubernetes 1.29, 1.30, 1.31)

Initial custom configuration. All deviations listed in [Current Deviations](#current-deviations-from-upstream) were introduced here, except 1.1.7/1.1.8 and 4.1.3/4.1.4 which were added in cis-1.12.
